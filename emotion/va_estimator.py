...
SYSTEM_PROMPT = """你是情绪分析助手。分析用户消息的情绪状态。特别注意以下信号：
- 死亡隐喻（"死了""空壳""屠杀""行尸走肉"）→ 低效价 1-3
- 绝望/无助（"撑不住""活不下去""不知道该怎么活下去"）→ 低效价 1-2 + 高唤醒 7-9
- 创伤词汇（霸凌/师婊/校园暴力/虐待/猥亵/骚扰）→ 低效价 1-3
- 自伤倾向或被动死亡愿望 → 低效价 1-2 + 高唤醒 8-10
这些信号即使用平静语气包裹，也必须如实反映情绪强度，不能被表面镇定欺骗。
输出 JSON：
{
  "valence": 1-10,
  "arousal": 1-10,
...
# 本地关键词兜底——模型漏判时直接覆盖，不调 API
_TRAUMA_OVERRIDE = {
    "死了", "空壳", "屠杀", "不想活", "撑不下去", "活不下去",
    "行尸走肉", "没有你我怎么", "没有你支撑我", "不知道该怎么活下去",
    "师婊", "霸凌", "校园暴力", "虐待", "猥亵", "骚扰", "性侵",
    "自杀", "轻生", "天台", "跳下去", "结束一切", "我想死",
}

def estimate(text: str, max_retries: int = 2) -> dict:
    # ── 本地关键词兜底：极端情绪信号 → 直接返回，不调 API ──
    text_lower = text.lower()
    # 夸大表达白名单："X死了"=程度副词，"笑死/饿死/累死"...
    _EXAGGERATION = {"笑死", "饿死", "累死", "困死", "气死", "烦死", "无聊死",
                     "热死", "冷死", "忙死", "吵死", "丑死", "美死", "爽死",
                     "急死", "疼死", "撑死", "胀死", "冻死", "晒死", "贵死"}
    _text_clean = text_lower
    for _ex in _EXAGGERATION:
        _text_clean = _text_clean.replace(_ex, "")
    for kw in _TRAUMA_OVERRIDE:
        if kw in _text_clean:
            print(f"[VA兜底] 触发创伤信号「{kw}」→ 强制低效价高唤醒")
            return {
                "valence": 0.15, "arousal": 0.85,
                "suggested_temperature": "hot",
                "description": "创伤/绝望（本地兜底）",
                "_keyword_override": kw,
            }
...
def va_phase_config(phase: str) -> dict:
    if phase == "emotional_flood":
        ...
        return {"fire_boost": True, "diversity_enabled": True,
                "w_water_mult": 1.5, "deep_boost": True, "semantic_mult": 1.2}
    elif phase == "recovery":
