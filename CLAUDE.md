...
通过 `ghost_cli.py` 在 Claude Code 内直接操作 ghost-trigger：

```bash
python ghost_cli.py status                      # 总览
python ghost_cli.py cards --cat deep_talks      # 查卡片
python ghost_cli.py cards --id <id>             # 查单张
python ghost_cli.py chat --recent 30            # 查聊天
python ghost_cli.py chat --search "关键词"       # 搜索聊天
python ghost_cli.py diary --days 3              # 查日记
python ghost_cli.py recall "查询"               # 模拟检索
