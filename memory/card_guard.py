...
_GARBAGE_TITLES = {
    ...
    "无新承诺", "不要死在那个夏天",  # auto-trigger 误生成的短语标题
}
...
def check_before_write(title: str, content: str, user_input: str) -> tuple[bool, str]:
    ...
    if os.path.exists(db_path):
        try:
            ...
            for oid, otitle, ocontent, oimp, ocat in c.fetchall():
                # 深层记忆卡不参与自动拦截
                ...
                if overlap >= 2 and (oimp or 5) < 8:
                    ...
                    if ocat in ('commitments', 'daily_life', 'todo'):
                        from memory.memory_manager import should_auto_resolve, resolve_card
                        allowed, reason = should_auto_resolve(oid)
                        if allowed and resolve_card(oid):
                            print(f"[写卡拦截-auto] 新卡「{title}」→ auto-resolve 旧卡「{otitle}」({overlap})")
                            conn.close()
                            return True, f"与旧卡「{otitle}」重叠({overlap})，已划旧卡"
                    else:
                        print(f"[写卡拦截-auto] 新卡「{title}」与{ocat}卡「{otitle}」重叠({overlap})，仅拦截不划")
                    conn.close()
                    return True, f"与{ocat}卡「{otitle}」重叠({overlap})，仅拦截"
            conn.close()
